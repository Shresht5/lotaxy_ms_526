...
export const registerIpcsHandler = () => {
    //user
    ipcMain.handle('db-get-users', () => getAllUsers());
    ipcMain.handle('db-add-user', (_, name, email, phone, department, position, date_of_joining, date_of_birth, total_experience, performance, potential, ctc) => addUser(name, email, phone ?? '', department ?? '', position ?? '', date_of_joining ?? '', date_of_birth ?? '', +total_experience || 0, performance ?? 'Average', potential ?? 'Medium', +ctc || 0));
    ipcMain.handle('db-update-user', (_, id, name, email, phone, department, position, date_of_joining, date_of_birth, total_experience, performance, potential, ctc) => updateUser(id, name, email, phone ?? '', department ?? '', position ?? '', date_of_joining ?? '', date_of_birth ?? '', +total_experience || 0, performance ?? 'Average', potential ?? 'Medium', +ctc || 0));
    ...
    ipcMain.handle('attendance-mark', (_, user_id, date, check_in, status, remark) => markAttendance(user_id, date, check_in, status ?? 'Present', remark ?? ''));
    ipcMain.handle('attendance-update', (_, id, check_in, status, remark) => updateAttendance(id, check_in, status, remark ?? ''));
    ipcMain.handle('attendance-delete', (_, id) => deleteAttendance(id));

    //client
    ipcMain.handle('db-get-clients', () => getAllClients());
    ipcMain.handle('db-add-client', (_, name, email, phone, address, company_name, status) => addClient(name, email, phone ?? '', address ?? '', company_name ?? '', status ?? 'Active'));
    ipcMain.handle('db-update-client', (_, id, name, email, phone, address, company_name, status) => updateClient(id, name, email, phone ?? '', address ?? '', company_name ?? '', status ?? 'Active'));
    ...
    ipcMain.handle('project-add', (_, name, client_id, detail, what_done, what_todo, completion, status) => addProject(name, client_id ?? null, detail ?? '', what_done ?? '', what_todo ?? '', completion ?? 0, status ?? 'Active'));
    ipcMain.handle('project-update', (_, id, name, client_id, detail, what_done, what_todo, completion, status) => updateProject(id, name, client_id ?? null, detail ?? '', what_done ?? '', what_todo ?? '', completion ?? 0, status ?? 'Active'));
